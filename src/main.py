...
class GenerateError:
	...
	def process_error(self, i, row, label, current, prev_error, last=False, eos=False):
		...
		if label in util.ptb_forward or last:  # hallucination/ptb sub/following word
			if not eos:
				try:
		...
		if label in util.ms_forward or last:  # miss/ms sub/following word
			if not eos:
				try:
